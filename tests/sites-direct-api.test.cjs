...
const advice = () => ({body:'Power needs a conduit.',observation:'An array stands by the colony.',taskSuggestion:'none',visualEvidence:null});
...
function harness({verify, infer}={}) {
  ...
  const context=vm.createContext({window,crypto:{randomUUID},document:{dispatchEvent:event => events.push(event.type)},CustomEvent:class {constructor(type){this.type=type;}},AbortController,Response,
  ...
  for (const file of ['astrabot-contract.js','astrabot-planning.js','astrabot-api.js']) vm.runInContext(fs.readFileSync(file === 'astrabot-api.js' ? path.join(project,'scripts/sites-direct-api.js') : path.join(project,'Assets/WebGLTemplates/Astra',file),'utf8'),context,{filename:file});
...
test('tab key goes only to OpenAI Authorization headers and Responses uses strict, stateless image input',async () => {
  ...
  const response=await client.request(); const received=await response.json(); assert.equal(received.actionId,'solar'); assert.equal(received.body,advice().body); assert.equal(received.visualEvidence,undefined);
...
for (const reason of ['forget','caller','timeout','replace']) test(`${reason} aborts inference and discards a late response`,async () => {
  ...
  if(reason === 'timeout') client.fire(40000);
...
for (const [name, value] of [
  ...
  ['invalid suggestion',envelope({...advice(),taskSuggestion:'shell'})],
  ['invalid body',envelope({...advice(),body:''})]
...
test('coaching withholds local target geometry and attaches only a local action afterwards',async () => {
  const client=harness(); await client.api.connect(KEY);
  const data=frame(); data.state.uiAnchors={secret:true}; data.state.frontier={x:20,y:20};
  await client.request(data);
  const context=JSON.parse(JSON.parse(client.calls.at(-1).options.body).input[0].content[0].text);
  assert.equal(context.candidates,undefined); assert.equal(context.state.frontier,undefined); assert.equal(context.state.uiAnchors,undefined);
});
test('conversation returns the browser NDJSON protocol without executing a delegated task',async () => {
  const client=harness({infer:() => result(envelope({reply:'I can prepare exploration.',intent:'task',goal:'Explore nearby terrain'}))});
  await client.api.connect(KEY);
  const response=await client.api.request('/api/astrabot/chat',{method:'POST',body:JSON.stringify({message:'Explore nearby',state:{session:'colony'},history:[]})});
  const event=JSON.parse(await response.text());
  assert.equal(event.type,'done'); assert.equal(event.intent,'task'); assert.equal(event.goal,'Explore nearby terrain');
