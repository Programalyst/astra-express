...
ANSWER = {'actionId': 'explore', 'title': 'Explore', 'body': 'Reach the fog.', 'observation': 'The rover is beside the colony.'}
CONFIG = {'key': 'private-test-value', 'model': 'gpt-5.4-mini'}
...
def payload():
    ...
    return {'image': 'data:image/jpeg;base64,' + base64.b64encode(b'\xff\xd8\xff' + b'0' * 200).decode(),
            ...
            'events': [], 'question': 'What next?'}


def events(answer=ANSWER, create=True, terminal='agent.session.turn.completed', phase='final_answer', turn='turn_1'):
    result = []
    if create:
...
class ProtocolTests(unittest.TestCase):
    ...
    def test_stale_actions_or_replacement_steps_are_rejected(self):
        for bad in [{**ANSWER, 'actionId': 'old-action'}, {**ANSWER, 'steps': ['Build instantly']}, {**ANSWER, 'body': None}]:
            with self.assertRaises(ValueError): coach.parse_advice(json.dumps(bad), payload())
        self.assertEqual(coach.parse_advice(json.dumps(ANSWER), payload()), ANSWER)
...
class LocalHTTPTests(unittest.TestCase):
    ...
    def test_request_reaches_agents_and_returns_validated_advice(self):
        ...
        with patch.object(self.server.agents, 'request', return_value=events()) as upstream:
            ...
            self.assertEqual(code, 200); self.assertEqual(json.loads(body), ANSWER)
