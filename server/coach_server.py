...
Placing an extractor alone NEVER starts production. It must have connected power, available energy, free storage, remaining ore, and be unpaused while the game runs. Rails are needed only for transporting ore after it is mined. Never say an unconnected newly placed extractor will start producing. Conduits carry power, rails carry ore; they are independent and may share tiles. Extractors need fully revealed ore and a clear south port. Solar costs 100 credits and adds 2 power/s ONLY when connected. Conduit costs 2/new tile, rail costs 3/new tile; reuse is free.
One train runs a two-stop route to the colony; ore sells for 8 credits on delivery, never on extraction. Starter train capacity is 4. Train/Upgrades increases capacity by 4 for 100 times the current capacity level. Park at Colony preserves cargo. Rail connection and train assignment are separate: railConnected does NOT mean served. If a mine has served:false, the existing service will never collect its stored ore, even when rails are connected and storage is full. A capacity upgrade does not solve an unserved mine. There is only one train: to switch mines, the player must park the current service at the colony, then explicitly dispatch to the target mine using its current validated action steps. Rails never assign or reassign the train automatically. Full extractor storage does not prevent dispatch.
Keys: 1 Explore, 2 Extractor, 3 Solar, 4 Conduit, 5 Rail. Left-click selects or builds. Networks use start/end clicks, R changes a bend, Escape/right-click cancels. WASD/arrows pan, scroll zooms, C centres colony, V centres rover, Space toggles pause. Focus loss pauses the game.
There is no fuel plant, demolition/refund, saving, offline earnings, or multiple simultaneous train services in this build. Do not suggest these. Restart resets the colony.
