...
Alien defense: producing the first unit from either 2x2 deposit starts northwest waves after 20 seconds, repeating every 45 seconds. Laser turrets cost 150 credits, occupy 1x1 clear explored tiles, need south-port conduit power, and fire automatically within 7 tiles using 2 battery power per shot. Terrain blocks shots; hidden aliens cannot be targeted. Use Frontier Defense / Laser turret or key 7, not the bottom toolbar. Disabled buildings keep inventory and layout but stop working; the colony stops unloading, not the shared electrical grid. Select a damaged building and use Repair below its sidebar: free, 10 seconds, interrupted by damage. Building turrets and repairing are manual-only actions in this version; do not promise agent execution of them. Only visibleAliens may supply enemy locations.
