...
def route_planner_model(data, astra_model, exploration_model):
    """Route only pure rover/fog discovery to the smaller model.

    Mixed, construction, transport, power, and ambiguous goals stay on Astra.
    The decision uses the bounded player goal only; model output cannot select
    its own model or downgrade a later construction step.
    """
    goal = data.get('goal', '') if isinstance(data, dict) else ''
    if isinstance(goal, str) and EXPLORATION_INTENT.search(goal) and not ADVANCED_INTENT.search(goal):
        return {'model': exploration_model, 'route': 'rover-exploration'}
    return {'model': astra_model, 'route': 'advanced-visual'}
...
def build_planner_request(data, model, game_rules):
    return {'agent': {'model': model, 'instructions': PLANNER_RULES + '\nGAME RULES:\n' + game_rules,
                      'tools': [], 'multi_agent': {'enabled': False}, 'reasoning': {'effort': 'low'},
...
def validate_actions(actions, data):
    ...
    ids = set()
    visual_repair = re.match(r'^Select the extractor at \(([0-9]+), ([0-9]+)\) and connect its south port\b', data.get('goal', ''))
    visual_repair_target = (int(visual_repair.group(1)), int(visual_repair.group(2))) if visual_repair else None
    for index, action in enumerate(actions):
        ...
        xy = point(action)
        if visual_repair_target:
            if kind not in ('select', 'connect_conduit', 'wait', 'stop'):
                raise ValueError('Visual repair plan contains an unrelated action')
            if kind in ('select', 'connect_conduit') and xy != visual_repair_target:
                raise ValueError('Visual repair action changed the diagnosed target')
