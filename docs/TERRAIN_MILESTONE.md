...
Sparse scenery: 24 seeded, randomly arranged instances of `Assets/Kenny/rock_largeA_var1.prefab` decorate the map with varied scale and rotation. The variant's authored scale and shared materials are preserved, with a 0.65–1.10 random scale multiplier rather than automatic bounds fitting. Sites have a one-cell clearance from cliffs, ramps, resources, and starter buildings, with at least two intervening cells between neighboring rocks. They follow elevation and fog, have no gameplay collision, and hide when their tile receives a building, rail, or conduit. They are scenery, not ore deposits; the layout is stable across restarts.

Editable cliff materials: `Assets/Materials/Terrain/Cliff Rock.mat` and `Cliff Rock Dark.mat` are shared URP Lit assets mapped to the Kenney cliff, corner, and ramp models' `rock`/`rockDark` slots. The double-cliff prefab inherits these assignments. Terrain instances preserve these materials directly instead of generating runtime copies, so material settings remain editable and consistent across the cliff pieces. Initial colors match the supplied pack; the separate desert ground material is unchanged.

Latest resource adjustment: the former 2x2 Fluxite is now Ore at `(25, 25)` on low ground north of the eastern plateau, with its south port at `(25, 24)`; its previous site `(22, 16)` is empty. The 2x2 Ore remains at the northern site `(4, 23)`, with its south port at `(4, 22)`. The farther 1x1 Ore moves from `(12, 16)` to `(23, 13)`, near the center of the eastern plateau's flat top, with its south port at `(23, 12)`. Its previous site is empty. Footprints, yields, and the other 1x1 deposits are unchanged. The eastern plateau is shortened to leave this deposit and its port on low ground. The nearby 1x1 ore remains at `(10, 4)`, with its extractor port at `(10, 3)`; its previous site `(13, 3)` is empty. The 1x1 Fluxite moves from `(13, 8)` to `(13, 13)`, with its south port at `(13, 12)`.

Current enclosed-plateau layout: the eastern plateau has cliff boundaries at columns 17/29 and rows 3/22, with flat upper ground at columns 18–28 and rows 4–21. Its western ramps remain `(17, 5)` and `(17, 15)`. A new north-facing descent at `(25, 22)` leads from upper ground `(25, 21)` through low ground `(25, 23)` to the ore extractor port `(25, 24)`. The smaller northern plateau has cliffs at columns 1/9 and rows 18/26, enclosing a 7x7 flat top at columns 2–8 and rows 19–25. Its south access moves to `(4, 18)`; a new east-facing descent at `(9, 22)` connects directly to the low valley between the plateaus. Both mesas have four corner pieces, four cliff sides interrupted only by ramps, and surrounding low ground. Resource coordinates are unchanged. Historical layouts follow below.
