...
Latest resource adjustment: the 2x2 Fluxite moves to `(25, 25)` on low ground north of the eastern plateau, with its south port at `(25, 24)`; its previous site `(22, 16)` is empty. The 2x2 Ore remains at the northern site `(4, 23)`, with its south port at `(4, 22)`. The farther 1x1 Ore moves from `(12, 16)` to `(23, 13)`, near the center of the eastern plateau's flat top, with its south port at `(23, 12)`. Its previous site is empty. Footprints, yields, and the other 1x1 deposits are unchanged. The eastern plateau is shortened to leave this deposit and its port on low ground. The nearby 1x1 ore remains at `(10, 4)`, with its extractor port at `(10, 3)`; its previous site `(13, 3)` is empty. The 1x1 Fluxite remains at `(13, 8)`.

Current enclosed-plateau layout: the eastern plateau has cliff boundaries at columns 17/29 and rows 3/22, with flat upper ground at columns 18–28 and rows 4–21. Its western ramps remain `(17, 5)` and `(17, 15)`. A new north-facing descent at `(25, 22)` leads from upper ground `(25, 21)` through low ground `(25, 23)` to the Fluxite extractor port `(25, 24)`. The smaller northern plateau has cliffs at columns 1/9 and rows 18/26, enclosing a 7x7 flat top at columns 2–8 and rows 19–25. Its south access moves to `(4, 18)`; a new east-facing descent at `(9, 22)` connects directly to the low valley between the plateaus. Both mesas have four corner pieces, four cliff sides interrupted only by ramps, and surrounding low ground. Resource coordinates are unchanged. Historical layouts follow below.
