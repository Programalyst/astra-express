...
For teammate setup, pull these source changes and open `Assets/Scenes/AstraExpress.unity` in Unity 6000.3.24f1. The existing bridge package dependency is an absolute local path in `Packages/manifest.json`; a different machine must point it at that teammate's updated bridge checkout before Unity can resolve packages. The gameplay world is generated on entering Play Mode, so no new scene or vendor-asset changes are required for this milestone.
