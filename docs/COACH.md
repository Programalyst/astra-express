...
For screen understanding, click the **gear immediately left of Copilot**, paste an OpenAI API key (the `sk-…` value only), and click **Connect for this tab**. The panel links to key creation and explains API billing. Connecting checks model access without generating a response. Pasted keys stay in tab memory and are sent only to the same-origin game server; they are never written to files, browser storage, game state, or model prompts. Reload, closing the tab, or **Forget key** clears the override. Forgetting a tab key falls back to any configured server key; use Copilot Off to stop help. Opening settings stops active takeover, and changing credentials invalidates pending plans.

Alternatively set `OPENAI_API_KEY` in the ignored `server/.env` (see `.env.example`). The server reads changes without restart. `ASTRA_COACH_MODEL` defaults to `gpt-5.4-mini`. Only `Builds/Web` is served. Pasted keys use isolated short-lived hosted Agents sessions, deleted after each turn, and separate bounded in-memory goal histories so exploration can continue across batches. The existing server-key session lifecycle remains unchanged. The key panel requires the coach backend; a static-only Sites export cannot authenticate requests by itself.
