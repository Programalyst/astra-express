...
That playtest also found a coaching candidate gap: a second powered, rail-connected mine had `served:false` and 24/24 storage, while the existing train served another mine. The old candidate set offered an unrelated capacity upgrade, so a valid structured reply still gave poor gameplay advice. The gameplay policy now supplies a mine-reassignment candidate, and the server instructions explicitly separate rail connectivity from service assignment: capacity upgrades cannot collect an unserved mine; park the single train at the colony and explicitly dispatch to the target. Full storage does not block dispatch. The next playtest checks that corrected advice and interaction.
