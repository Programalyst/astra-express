...
> **Local and public transports differ:** local coaching and planning use `server/coach_server.py` and hosted Agents sessions (`gpt-6-astra`, with `gpt-5.4-mini` for exploration). The static public package uses `--direct-api` to substitute deployment-only browser adapters: optional player keys call OpenAI Responses with `gpt-5.4-mini`; no key is needed for local hints. No Python service or shared key is deployed. Direct Responses results never display Astra Agents visual-evidence badges.

## Live site

Production URL: https://astra-express.leonard-lin-2003.chatgpt.site

Latest release: version 12, succeeded at 21:20 Singapore time on 14 September 2026 (deployment `appgdep_6aa7f48a6d2c8191b8cb87e433f42146`). Deployment source: `836aa1ccb343e0ba88b9c31343a49e3207872edd`; Unity checkpoint `31091f3` plus the browser planner's 32×32 bounds correction. Includes revised deposits and enclosed plateaus, editable cliff colors, sparse rock variants, and optional direct-key chat/coaching/plans alongside local hints. Compressed payload: 28,054,024 bytes in 15 pieces. Release entry: `play-ac9dd1302f67f1e0.html`. Rebuilt player and key settings inspected locally at 1336×768. All 161 JavaScript and 82 Python tests pass, together with simulation, exploration, and bot-control checks. Direct transport tests use mocked responses, not real inference or visual-understanding evidence. Public root references the new adapter, which matches the local source byte-for-byte; real-key inference was not exercised.

Previous release: version 11, reported succeeded at 23:05 Singapore time on 13 September 2026 (deployment `appgdep_6aa6bb943dc48191a7938782477f7a73`). Deployment source: `d3b1659ceb5f5fc4e72edcf7436a408994bec220`; Unity checkpoint: `372c900`, with a clean source checkout at export. Adds the Martian splash-art loading backdrop and automatic train dispatch for connected mining services, retaining the previous visual/gameplay upgrades. Web export succeeded in 23 seconds. Compressed payload: 27,637,185 bytes in 15 pieces; the separate backdrop JPEG is 465,587 bytes and content-fingerprinted. Release entry: `play-ddaf3641eb4215b5.html`. The loading screen was visually checked locally and its packaging regression test passed before this deployment. Existing public access and browser-provided API keys are unchanged. Publication is confirmed by Sites; no additional live browser playtest was performed for this release.
...
The default Web template requires the same-origin Agents server. For static publication, `--direct-api` substitutes `scripts/sites-direct-api.js` and `scripts/sites-direct-settings.js` while preserving the current contracts, chat UI, and bounded planning validator. Chat replies use the browser's NDJSON protocol; delegated tasks still require explicit Start. Keys remain in tab memory, are cleared on reload/Forget, and go only to OpenAI Authorization headers. Use restricted, low-budget keys: browser code can access tab memory, so this is not a secure store for production credentials. `--local-hints-only` is an alternative without AI or key entry; never combine the flags. Neither flag changes local hosted Agents behavior. See [COACH.md](COACH.md) for local setup.
...
2. Run `node scripts/prepare-sites-direct.mjs --direct-api` from the Unity repository root to retain optional player API keys on static hosting.
