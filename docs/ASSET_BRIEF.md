...
The playable floor uses the Space Kit's `terrain.fbx`, `terrain_ramp.fbx`, `terrain_sideCorner.fbx`, and the user-authored `terrain_sideCliff_double.prefab` (two adjacent `terrain_sideCliff.fbx` models), retaining their rock materials through URP conversion. The double-cliff prefab replaces `terrain_side.fbx` for blocked hillside walls. Terrain bounds are centered horizontally and grounded automatically, then slopes and cliffs are fitted per axis to a 2x2-unit cell with a 1.5-unit rise; the prefab's original pivot does not need editing. The eastern plateau has east-facing uphill passes. The northern Fluxite plateau has a north-facing uphill pass, a west-facing eastern hillside, and an outer corner joining its edges. Flat tiles retain narrow grid seams; slope edges meet adjacent levels. Plateau bedrock fills the exposed outer edge. Fog replaces every terrain material slot until its cell is explored. Terrain-only mesh colliders support accurate picking, while movement samples the matching simulation height profile on walkable cells. The cliff FBX must remain CPU-readable for Web picking.
