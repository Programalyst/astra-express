...
The colony uses `Assets/Synty/PolygonSciFiWorlds/Prefabs/Buildings/SM_Bld_Pod_Research_05.prefab` from the owner's Asset Preview project. Its URP materials, transparent glass, and emission are preserved. Uniform fitting retains the colony's 2×2 footprint, foundation, location, and south power/rail port. Only visual dependencies are imported; legacy physics colliders are omitted.

Fluxite deposits use the owner's `Assets/Imported/NuclearKnights/Prefabs/Environment/Single Crystal.prefab`, assigned through `Fluxite Model` on Astra Express. Each deposit cell gets one crystal prefab, uniformly fitted within 1.55 units horizontally and 1.4 units vertically, grounded at its terrain elevation. Original prefab materials are preserved for manual color/emission edits; ordinary ore keeps its Kenney rocks. Resource yields, deposit footprints, and fog visibility are unchanged; revealed resources remain visible after an extractor is placed.

Extractors use the owner's `SM_Prop_Drill_03.prefab` from the same Environment folder. One drill is centered and uniformly scaled over the deposit footprint, with its original materials and motor animation preserved. Extractors have no opaque foundation or generated processing tower so resources remain visible beneath them. Animation speed follows the extractor's supplied-power fraction; disconnected, paused, full-storage, and globally paused extractors stop. Power plants retain their previous generator model through the separate `Power Plant Model` reference.
