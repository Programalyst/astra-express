...
- The current local browser build uses the same-origin Python hosted Agents API bridge. Advisory coaching and advanced construction plans use `gpt-6-astra`; pure rover/fog/ore-discovery plans route to `gpt-5.4-mini`. Eligible continuations of narrowly constrained mine-and-solar goals may use verified game state without another model call. The public Sites release may still be the older direct Responses build. Keep deployed and local claims separate.
- Astra coaching can offer reviewable follow-up goals: Ore discovery can prefill a rover survey, while mining/base-expansion language can prefill powered mining outposts around the one fixed colony. Suggestions must open the task composer for review and must not plan, spend, or start automatically.
...
- Keep the model evidence box distinct from the local action crosshair. `GROUNDED` means only that the box contains the withheld target point within the configured tolerance; `CHECK` must remain visible for misses or unavailable comparisons. Neither label alone proves that every object name or diagnosis is correct.
- Expose actual capability provenance for model coaching: model, Agents API backend, image-plus-state mode, measured latency, frame age, and action origin. Do not attribute verified local continuations to a new model call.
