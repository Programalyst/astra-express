(() => {
  ...
  panel.innerHTML = `<header><div><span class="settings-eyebrow">COLONY COPILOT</span><h2 id="astrabot-settings-title">Connect AstraBot</h2></div><button type="button" id="astrabot-settings-close" aria-label="Close AstraBot settings">×</button></header>
    ...
    <p id="astrabot-key-privacy" class="settings-note">Tab only · reload or Forget key clears it. Sent directly from this browser to OpenAI; never saved to files or browser storage. Use a restricted, low-budget key, not a production key.</p>
  ...
  async function refresh() {
    ...
    } catch { el("astrabot-key-source").textContent = "OpenAI connection unavailable · local game tips still work"; }
  }
  function show(value) {
    if (value) {
  ...
  el("astrabot-key-form").onsubmit = async event => {
    ...
    el("astrabot-key-connect").disabled = true; message("Checking model access · no generation…");
    try {
  ...
  el("astrabot-key-forget").onclick = () => {
    ...
    message("Tab key cleared. Local hints remain available without a key."); refresh();
  };
