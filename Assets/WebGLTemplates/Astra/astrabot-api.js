(() => {
  ...
  async function config(options = {}) {
    ...
    if (!response.ok) throw new Error("AstraBot server unavailable. Start the game with its coach server.");
    const data = await response.json();
    if (typeof data.token !== "string" || data.engine !== "agents-api") throw new Error("This game needs the AstraBot server to connect a key.");
  ...
  async function connect(value) {
    ...
    try {
      ...
      if (version !== revision || controller.signal.aborted) throw cancelled();
      if (!response.ok) throw new Error(response.status === 401 ? "OpenAI rejected this key. Check it or create a new one." : response.status === 403 ? "This key needs access to the model. Check its project permissions." : response.status === 429 ? "Too many attempts or an OpenAI limit. Try again shortly." : "Could not verify this key. Check your connection and try again.");
